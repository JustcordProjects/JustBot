...
export default function registerLeveling() {
    client.on('messageCreate', async (msg) => {
        ...
        if (!account) {
            ...
            await msg.reply(translate("hej!\nleveling oraz wiele innych rzeczy jest tu zarządzanych przez bota discord, który został przeportowany na serchat! moze dolaczysz na discorda (link w kategorii ważne) i połączysz discorda z serchatem przez komende `manage-accounts` z discorda by nabijał sie level?\n-# ta wiadomość nie pokaże się ponownie do następnego restartu bota"));
